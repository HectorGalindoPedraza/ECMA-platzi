...
```
El problema es que ```Promise.all()``` se resolverá, si y solo si **todas las promesas fueron resultas**. Si al menos una promesa es rechazada, ```Promise.all``` será rechazada.
### Promise.allSettled
